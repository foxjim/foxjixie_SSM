							...
							<fieldset>
								...
								<div class="form-group">
									...
									<div class="col-sm-8">
										...
										<select name="userCollege" id="userCollege" class="register_select" required="required" oninput="ensureCollege()">
											<option selected disabled value="">----请选择专业----</option>
										</select>
								...
								<div class="form-group">
									...
									<div class="col-sm-8" style="clear: none;">
										<!-- 邮箱文本框 -->
										<input type="text" name="email" class="validate-password required " readonly="readonly" oninput="ensureEmail()"
...
<script type="text/javascript">
	...
	//从后台获取所有学院显示在select下拉列表-后续更新显示
	setInterval(function earnUserCollege(){
		var userCollege=document.getElementById('userCollege');
		var userChoose=userCollege.value;
			...
			success:function(result){
				if(jsonnum!=result){
					...
					if(userCollege.value.length<=0){
						userCollege.innerHTML='<option selected disabled value="">----请选择学院----</option>';
					}else{
						userCollege.innerHTML='<option disabled value="">----请选择学院----</option>';
					}
					...
					for(var i in colleges){
						...
						option.value=colleges[i].collegeNo;
						option.innerHTML=colleges[i].collegeName;
						if(colleges[i].collegeNo==userChoose){
							option.selected='selected';
						}
					}
					window.confirm('学院下拉列表已更新');
				}
